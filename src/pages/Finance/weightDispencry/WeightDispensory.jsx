...
const WeightDispensory = () => {
  ...
  const onSelectChange = (newSelectedRowKeys) => {
    ...
    const selectedData = weightDispensory?.data?.find(
      (item) => item._id === newSelectedRowKeys[0]
    );
  ...
  const openNumber = weightDispensory?.data?.filter(
    (amt) => amt.status === "open"
  );
  const closedNumber = weightDispensory?.data?.filter(
    (amt) => amt.status === "closed"
  );
  const actionRequiredNumber = weightDispensory?.data?.filter(
    (amt) => amt.status === "action required"
  );

  const dataSourceWithKeys =
    weightDispensory?.data?.map((data) => ({
      ...data,
      key: data._id,
    })) || [];
  ...
  return (
    <div>
      ...
      >
        ...
        <SearchSellerModal
          weightDispensory={weightDispensory}
      ...
      >
        Disputes can be raised within 7 days of uploading the discrepancy. After
        that, it will be auto-accepted.
      </div>
    </div>
  );
};
