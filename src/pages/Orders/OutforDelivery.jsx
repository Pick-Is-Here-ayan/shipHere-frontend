...
const OutForDelivery = ({
  ...
  const columns = [
    ...
    {
      title: "Order Date",
      dataIndex: "updatedAt", // Change dataIndex to statusUpdateDate
      ...getColumnSearchProps("updatedAt"), // Ensure search works with statusUpdateDate
      filterDropdown: ({
        setSelectedKeys,
        selectedKeys,
        confirm,
      ...
      onFilter: (value, record) => {
        ...
        const orderDate = moment(record.updatedAt).toISOString(); // Update this to use statusUpdateDate
      ...
      render: (text, order) => {
        ...
        return (
          <>
            <div>
              {moment(order.updatedAt, "YYYY-MM-DD").format("DD-MM-YYYY")}{" "}
              <span style={{ marginLeft: "10px", fontStyle: "italic" }}>
                {moment(order.updatedAt, "HH:mm:ss").format("HH:mm:ss")}
              </span>
            </div>
          </>
        );
  ...
  //     (order) => order?.shipmentDetails.status_code === "121"
  //   );
  const shippedOrders = dataSource?.filter(
    (order) =>
